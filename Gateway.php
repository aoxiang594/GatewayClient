...
class Gateway
{
    ...
    public static function getClientIdListByGroup($group)
    {
        ...
        $data          = static::select(['uid'], ['groups' => is_array($group) ? $group : [$group]]);
        $client_id_map = [];
     ...
     * @return array
     */
    protected static function select($fields = ['session', 'uid', 'groups'], $where = [])
    {
        $t                        = microtime(true);
        $gateway_data             = GatewayProtocol::$empty;
    ...
    protected static function getAllGatewayAddressesFromRegister()
    {
        ...
        if (self::$disableCache || (empty($addresses_cache) || $time_now - $last_update > $expiration_time)) {
